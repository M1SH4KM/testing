...
local function createBlip(options)
    ...
    AddTextComponentString(options.title or 'Sin título dado')
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            ...
            if current.drivingschool then
                opts = {
                    label = 'Tomar lecciones de manejo',
                    ...
                    if isLoggedIn and closestCityhall and closestDrivingSchool then
                        if inside then
                            if current.drivingschool then
                                ...
                                exports['qb-core']:DrawText('[E] Tomar lecciones de manejo')
                            elseif current.cityhall then
                                inRangeCityhall = true
                                exports['qb-core']:DrawText('[E] Menú Empleos')
                            end
                        else
...
RegisterNetEvent('qb-cityhall:client:sendDriverEmail', function(charinfo)
    SetTimeout(math.random(2500, 4000), function()
        ...
        TriggerServerEvent('jpr-phonesystem:server:sendEmail', {
            Enviado = Lang:t('email.sender'),
            Assunto = Lang:t('email.subject'),
...
RegisterNUICallback('close', function(_, cb)
    ...
    if not Config.UseTarget and inRangeCityhall then exports['qb-core']:DrawText('[E] Menú Empleos') end -- Reopen interaction when you're still inside the zone
    cb('ok')
end)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        ...
        QBCore.Functions.Notify(('Recibiste tu %s por $%s'):format(license.label, id.cost), 'success', 3500)
    else
